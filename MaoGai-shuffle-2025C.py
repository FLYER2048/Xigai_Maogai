...
def newTK():
    ...
    for ls0 in ls:
        ...
        if previous_question_type != question_type:
            ...
            with open(folder_path+'\\25春-毛概题库（乱序版）.txt', 'a', encoding='utf-8') as file:
                file.write(f'{question_type[3:]}\n')
            with open(folder_path+'\\25春-毛概题库（乱序版）答案.txt', 'a', encoding='utf-8') as file:
                file.write(f'{question_type[3:]}\n')
        question = question.replace("(", "（").replace(")", "）")
        option = [_[3:] for _ in option.split('\n')]
        correct_index = [ord(s[0])-65 for s in correct_answer.split('\n')]
...
def oriTK():
    ...
    for ls0 in ls:
        ...
        if previous_question_type != question_type:
            cnt = 0
            previous_question_type = question_type
            with open(folder_path+'\\25春-毛概题库（顺序版）.txt', 'a', encoding='utf-8') as file:
                file.write(f'{question_type[3:]}\n')
            with open(folder_path+'\\25春-毛概题库（顺序版）答案.txt', 'a', encoding='utf-8') as file:
                file.write(f'\n{question_type[3:]}')
...
if __name__ == '__main__':
    print('请选择模式：1-无限刷题；2-输出练习题；3-输出原题')
    cmd = input()
    if cmd == '1':
        practice()
    elif cmd == '2':
